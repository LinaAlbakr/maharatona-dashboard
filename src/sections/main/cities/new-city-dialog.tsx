...
import { newCity } from 'src/actions/cities-and-neighborhoods';
...
export function NewCityDialog({ open, onClose }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      await newCity(data);
      enqueueSnackbar(t('MESSAGE.CITY_CREATED_SUCCESSFULLY'));
  ...
  return (
    <Dialog fullWidth maxWidth="sm" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 2 }}>{t('LABEL.NEW_CITY')}</DialogTitle>
